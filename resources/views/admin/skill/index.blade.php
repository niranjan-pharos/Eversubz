...
<style>
                ...
                <div class="row">
                    <div class="col float-right ml-auto">
                        <button class="btn btn-primary add-btn add-button" data-bs-toggle="modal"
                            data-bs-target="#addModalSkill"><i class="fa fa-plus"></i> Add Skill </button>
                    </div>
                    <div class="col-md-12">
                        <div class="card mb-0">
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table custom-table mb-0" id="skillTable">
                                        <thead>
                                            <tr>
                                                <th>Name</th>
                                                <th>Slug</th>
    ...
    <div class="modal custom-modal fade" tabindex="-1" role="dialog" id="editModal">
        <div class="modal-dialog" role="document">
            ...
            <form class="form form-validate" role="form" action="{{ route('skillUpdate') }}" method="post" id="updateForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="col-md-12">
                    <div class="row">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header remove">
                    ...
                    <h4 class="modal-title">Remove Skill</h4>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>

                <form role="form" action="{{ route('skillDelete') }}" method="post" id="removeForm">
                    @csrf
                    @method('DELETE')
                    <div class="modal-body">                   
                    ...
                    <div class="modal-footer modal-footer-uniform">
                        ...
                        <button type="submit" class="btn btn-danger submit-btn">Delete Skill</button>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === true) {
                        ...
                        $("#addModalSkill").modal('hide');

                        $("#add_skill")[0].reset();
                        $("#add_skill .form-group").removeClass('has-error').removeClass('has-success');
    ...
    function editFunc(id) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function(response) {
                $("#edit_name").val(response.skill.skill_name);
                if (response.skill.status === 0) {
                    $("#edit_status").val('0'); 
                } else {
                    $("#edit_status").val('1');
                }
