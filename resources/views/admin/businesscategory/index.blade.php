...
@section('content')
                ...
                <div class="row">
                    <div class="col float-right ml-auto">
                        <button class="btn btn-primary add-btn add-button" data-bs-toggle="modal"
                            data-bs-target="#addModalBusinessCategory"><i class="fa fa-plus"></i> Add Business Category </button>
                    </div>
                    <div class="col-md-12">
                        <div class="card mb-0">
                            <div class="card-body">
    ...
    <!-- edit modal -->
    <div class="modal custom-modal fade" tabindex="-1" role="dialog" id="editBusinessForm">
        <div class="modal-dialog" role="document">
            ...
            <form class="form form-validate" role="form" action="{{ route('businessCategoryUpdate') }}" method="post" id="updateBusinessForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="col-md-12">
                    <div class="row">
    ...
    <div class="modal custom-modal fade" tabindex="-1" role="dialog" id="removeModal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form role="form" action="{{ route('businessCategoryDelete') }}" method="post" id="removeForm">
                    @csrf
                    @method('DELETE')
                    <div class="modal-body">                   
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            categoryTable = $('#categoryTable').DataTable({
                'ajax': "{{ route('businessCategoryList') }}",
                'order': []
            });
            ...
            $('body').on('click','.change-status',function(){
                ...
                let id= $(this).data('id');
                $.ajax({
                    url: "{{route('businessCategory.change-status')}}",
                    method : 'PUT',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        status: isCHecked,
            ...
            $("#add_businesscategory").unbind('submit').on('submit', function(event) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === true) {
                            ...
                            $("#addModalBusinessCategory").modal('hide');

                            $("#add_businesscategory")[0].reset();
                            $("#add_businesscategory .form-group").removeClass('has-error').removeClass('has-success');
        ...
        function editBusCatFunc(id) {
            var baseUrl = '{{ url('/') }}';
            $.ajax({
                url: baseUrl + "/admin/business-category/" + id + "/edit",
                type: 'get',
                dataType: 'json',
                success: function(response) {
                    ...
                    $("#updateBusinessForm").unbind('submit').bind('submit', function() {
                        event.preventDefault(); 
                        var form = $(this); 
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if (response.success === true) {
                                    toastr.success(response.messages);
                                    $("#editBusinessForm").modal('hide');
                                    $("#updateBusinessForm .form-group").removeClass('has-error').removeClass('has-success');
...
@include('admin.businesscategory.create')
@endsection
