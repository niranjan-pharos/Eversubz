...
@section('title',  "my Business Products")
...
<style>
.product-type1{ top: 45px;}
  .product-type1 span{ background: yellow;color: rgb(204, 3, 3)}
  body{background:#F9FAFB}.product-widget,.product-card{background:#fff}.product-img{overflow:hidden;display:flex;justify-content:center}.product-category{height:auto;padding:0;margin:0}.product-content{padding:.875rem}.product-content1{display:flex;justify-content:space-between}.product-category .breadcrumb-item a{color:#0d9488}.product-title{height:auto;font-size:.85rem}.product-card-column{padding:0 5px}.product-card{margin:0 0 10px}.product-price{font-size:14px}.product-title.mobile{display:none}@media (max-width:575px){.product-title.mobile{display:block}.product-title.desktop{display:none}.product-content1{flex-direction:column}.product-meta span{font-size:11px}.expiry-date{float:left}.product-meta{margin-bottom:12px;column-gap:17px;display:flex}.product-card-column{margin-bottom:0;padding:0 5px}.product-card{width:auto;margin:0 auto 10px;height:auto}.product-media{text-align:center}.product-img{overflow:hidden;height:130px;align-content:center;background:#fff}.product-img img{width:100%;height:100%}.product-category{margin-bottom:1px;line-height:17px}.product-title{font-weight:600;font-size:13px;line-height:20px}.product-meta{margin-bottom:0;line-height:20px}.product-info{padding:10px 0;display:flex;align-items:center;justify-content:space-between;border-top:1px solid var(--border)}.product-status{margin-bottom:10px}.product-btn-group{display:flex;justify-content:space-around;align-items:center;margin-top:10px}.product-btn-group button{margin:0 5px;padding:5px 10px;font-size:.9rem}.product-btn{margin-top:10px}}
  </style>
<section class="myads-part">
   <div class="container">
            ...
            <div class="header-filter">
               ...
               <div class="filter-short">
                  ...
                  <select class="custom-select filter-select" id="postFilter">
                     <option selected data-filter="all" {{ request()->query('filter') == 'all' ? 'selected' : '' }}>All Ads</option>
                     <option value="3" data-filter="Booking" {{ request()->query('filter') == 'Booking' ? 'selected' : '' }}>Booking Ads</option>
                     <option value="2" data-filter="Rent" {{ request()->query('filter') == 'Rent' ? 'selected' : '' }}>Rental Ads</option>
                     <option value="1" data-filter="Sale" {{ request()->query('filter') == 'Sale' ? 'selected' : '' }}>Sale Ads</option>
                 </select>                 
               </div>
            </div>
         </div>
      ...
      {{-- pagination --}}
      @include('components.custom_pagination', ['paginator' => $products])
...
<script>
 $(document).ready(function(){$('.deleteForm').on('submit',function(e){e.preventDefault();var form=$(this);var deleteButton=form.find('.deleteButton');if(confirm('Are you sure you want to delete this item?')){deleteButton.attr('disabled',!0);$.ajax({url:form.attr('action'),type:'POST',data:form.serialize(),success:function(response){toastr.success('Product deleted successfully!');window.location.reload()},error:function(xhr){toastr.error('An error occurred. Please try again.')},complete:function(){deleteButton.attr('disabled',!1)}})}})});$(document).ready(function(){$('#resultsPerPage').on('change',function(){var resultsPerPage=$(this).val();var currentUrl=window.location.href;var updatedUrl=updateQueryStringParameter(currentUrl,'per_page',resultsPerPage);window.location.href=updatedUrl});$('#postFilter').on('change',function(){var filterValue=$(this).find(':selected').data('filter');var currentUrl=window.location.href;var updatedUrl=updateQueryStringParameter(currentUrl,'filter',filterValue);window.location.href=updatedUrl});function updateQueryStringParameter(url,key,value){var re=new RegExp("([?&])"+key+"=.*?(&|$)","i");var separator=url.indexOf('?')!==-1?"&":"?";if(url.match(re)){return url.replace(re,'$1'+key+"="+value+'$2')}else{return url+separator+key+"="+value}}});$(document).ready(function(){var urlSearchParams=new URLSearchParams(window.location.search);var perPageParam=urlSearchParams.get('per_page');if(perPageParam){$('#resultsPerPage').val(perPageParam)}})
</script>
