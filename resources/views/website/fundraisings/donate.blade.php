                        ...
                        <div>
                            ...
                            <div class="font-normal text-gray-500 mt-2 text-sm /70">
                                ...
                                <a href="{{ route('ngo.show', ['id' => $encryptedNgoId]) }}"
                                    ...
                                    {{ optional($fundraising->ngo)->ngo_name ?? 'N/A' }}
                                 </a>                                 
                            </div>
                        </div>
            ...
            <form id="donation-form">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <label for="amount" class="form-label labelheading mt-1 mb-1 text-black">Select an amount to support</label>
                            ...
                            <div class="row mt-1">
                                <div class="col-12 col-md-4 amount-input-container">
                                    <input type="number" class="form-control custom-input widthforamountcard" id="amount" placeholder="Enter Amount in AUD" step="1" min="0">
                                    @error('amount')
                                        <div class="text-danger mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateAmountAndTipSections() {
        const tipValue = (tipSwitcher.value / 100) * selectedAmount;
        const transactionFee = coverTransactionCostsCheckbox.checked ? (selectedAmount * 0.029) + 0.3 : 0;
        const totalAmount = selectedAmount + tipValue + transactionFee;
        ...
        if (coverTransactionCostsCheckbox.checked) {
            ...
            document.getElementById('transaction-fee-section').style.display = 'block';
        } else {
    ...
    if (coverTransactionCostsCheckbox) {
        const transactionFeeSection = document.getElementById('transaction-fee-section');
        const mainAmountsSection = document.getElementById('main_amounts_section');
        const valueDisplays = document.querySelectorAll('.value-display');
        const totalEl = document.getElementById('total');

        let mobileFlexStyle = document.getElementById('mobile-flex-wrap-style');
        if (!mobileFlexStyle) {
            mobileFlexStyle = document.createElement('style');
            mobileFlexStyle.id = 'mobile-flex-wrap-style';
            document.head.appendChild(mobileFlexStyle);
        }
        const applyMobileRule = () => {
            mobileFlexStyle.textContent = `
            @media only screen and (max-width: 767px) {
                .flex-wrap {
                flex-wrap: nowrap !important;
                }
        ...
        const syncUI = function() {
            ...
            if (transactionFeeSection) {
            transactionFeeSection.style.display = checked ? 'flex' : 'none';
            }

            if (mainAmountsSection) {
            if (checked) {
                mainAmountsSection.classList.remove('gap-4');
                mainAmountsSection.classList.remove('flex-wrap');
                mainAmountsSection.classList.add('flex-col');
            } else {
                mainAmountsSection.classList.add('gap-4');
                mainAmountsSection.classList.add('flex-wrap');
                mainAmountsSection.classList.remove('flex-col');
            }
            }

            valueDisplays.forEach(el => {
            el.style.width = checked ? '100%' : 'auto';
            el.style.display = checked ? 'flex' : 'block';
            ...
            if (totalEl) {
                if (checked) {
                    totalEl.style.borderTop = '2px solid #ddd';
                    totalEl.style.paddingTop = '5px';
                    totalEl.style.fontWeight = '600';
                    totalEl.style.color = '#2b54a4';
                    totalEl.style.fontSize = '18px';

                    totalEl.style.borderLeft = '';
                    totalEl.style.paddingLeft = '';
                } else {
                    totalEl.style.borderLeft = '2px solid #ddd';
                    totalEl.style.paddingLeft = '12px';
                    totalEl.style.fontWeight = '600';
                    totalEl.style.color = '#2b54a4';

                    totalEl.style.borderTop = '';
                    totalEl.style.paddingTop = '';
                    totalEl.style.fontSize = '';
                }
            }

            if (typeof updateAmountAndTipSections === 'function') {
            updateAmountAndTipSections();
            }
        };

        
        coverTransactionCostsCheckbox.addEventListener('change', syncUI);
    }
    ...
    donateButton.addEventListener('click', async (e) => {
        e.preventDefault();
            ...
            if (field) {
                ...
                console.error(`Field with name ${fieldId} not found in the DOM`);
            }
        });
        ...
        if (!isValid) {
            ...
            try {
                ...
                nonce = result.token;
                const anonymousCheckbox = form.querySelector('[name="anonymous"]');
                const anonymous = anonymousCheckbox.checked ? 1 : 0;
                const payload = {
                    nonce,
                ...
                if (data.success) {
                    ...
                    document.getElementById('tip-switcher').value = 0; 
                    ...
                    if (paymentForm) {
                        const card = Square.payments(appId, locationId).card();
                        card.clear();
                    }
                    ...
                    toastr.error(data.message || 'An error occurred during donation.');
                }
            } catch (error) {
                ...
                donateButton.disabled = false;
                const loader = donateButton.querySelector('.spinner-border');
